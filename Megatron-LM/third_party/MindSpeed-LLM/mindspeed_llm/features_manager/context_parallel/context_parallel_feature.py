...
from mindspeed.features_manager.context_parallel.context_parallel_feature import (
    ContextParallelFeature as MindspeedContextParallelFeature,
)


def _is_deepseek4_spec(args):
    spec = getattr(args, 'spec', None)
    if spec is None:
        return False
    if isinstance(spec, str):
        spec = [spec]
    return any('deepseek4_spec' in str(item) for item in spec)
...
class ContextParallelFeature(MindspeedContextParallelFeature):
    def register_args(self, parser: ArgumentParser):
        ...
        group.add_argument(
            "--use-fused-ring-attention-update", action='store_true', help="Use fused ring attention update."
        )
        group.add_argument("--megatron-cp-in-bnsd", action='store_true', help="Megatron CP in bnsd.")
    ...
    def register_patches(self, patch_manager, args):
        if int(getattr(args, 'context_parallel_size', 1)) > 1 and not getattr(args, 'reset_attention_mask', None):
            ...
            patch_manager.register_patch(
                'megatron.core.parallel_state.initialize_model_parallel', initialize_model_parallel_cp_wrapper
            )
            patch_manager.register_patch(
                'megatron.core.parallel_state.destroy_model_parallel', destroy_model_parallel_cp_wrapper
            )
            patch_manager.register_patch(
            ...
            patch_manager.register_patch(
                'megatron.core.transformer.attention.Attention.__init__', attention_init_wrapper
            )
            patch_manager.register_patch(
                'megatron.core.transformer.dot_product_attention.DotProductAttention', CPDotProductAttention
            )
