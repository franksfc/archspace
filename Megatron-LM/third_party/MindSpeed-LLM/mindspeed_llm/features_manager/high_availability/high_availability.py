...
class HighAvailabilityFeature(MindSpeedFeature):
    def __init__(self):
        super().__init__(feature_name='high-availability', optimization_level=0)

    def register_args(self, parser: ArgumentParser):
        group = parser.add_argument_group(title=self.feature_name)
        group.add_argument(
            '--enable-high-availability', action='store_true', help='switch of the high availability feature'
        )
        group.add_argument(
            '--enable-hbmfault-repair', action='store_true', help='high availability feature, enable hbmfault repair'
        )
        group.add_argument(
            '--enable-worker-reboot', action='store_true', help='high availability feature, enable worker reboot'
        )
        group.add_argument(
        ...
        group.add_argument(
            '--enable-elastic-training', action='store_true', help='high availability feature, enable elastic training'
        )
    ...
    def validate_args(self, args):
        if args.enable_high_availability:
            if importlib.util.find_spec("mindio_ttp") is None:
                raise AssertionError("High availability feature requires the mindio_ttp package but is not installed.")
    ...
    def pre_register_patches(self, patch_manager, args):
        ...
        patch_manager.register_patch('torch.distributed.barrier', barrier_wrapper)
        for communication in ['all_reduce', '_all_gather_base', 'broadcast', 'all_gather_into_tensor']:
            patch_manager.register_patch('torch.distributed.distributed_c10d.' + communication, communication_wrapper)
        from mindspeed_llm.tasks.high_availability.communication_patch import (
            group_index_two_torch_wrapper,
            group_index_three_torch_wrapper,
            all_to_all_single_wrapper,
        )

        patch_manager.register_patch('torch.distributed.all_to_all_single', all_to_all_single_wrapper)
        for communication in [
            ...
            patch_manager.register_patch('torch.distributed.' + communication, group_index_two_torch_wrapper)
        for communication in [
            'gather',
            'scatter',
            'reduce',
            ...
            patch_manager.register_patch('torch.distributed.' + communication, group_index_three_torch_wrapper)
    ...
    def register_patches(self, patch_manager, args):
        ...
        from mindspeed_llm.core import (
        ...
        if args.enable_high_availability:
            ...
            patch_manager.register_patch(
                'megatron.training.initialize._initialize_distributed', initialize_distributed_wrapper
            )
            patch_manager.register_patch(
                'megatron.core.optimizer.clip_grads.get_grad_norm_fp32', get_grad_norm_fp32_wrapper
            )
            patch_manager.register_patch(
            ...
            patch_manager.register_patch(
                'megatron.core.optimizer.optimizer.ChainedOptimizer.load_parameter_state',
                chained_optimizer_load_parameter_state_patch,
            )
            patch_manager.register_patch(
                'megatron.core.optimizer.distrib_optimizer.DistributedOptimizer.load_parameter_state',
                distrib_optimizer_load_parameter_state_patch,
            )
            patch_manager.register_patch(
                'megatron.training.checkpointing._load_base_checkpoint', checkpointing_load_base_checkpoint_patch
            )
            patch_manager.register_patch(
                'megatron.core.parallel_state.initialize_model_parallel', initialize_model_parallel_wrapper
            )
            if args.reuse_fp32_param:
                ...
                patch_manager.register_patch(
                    'mindspeed_llm.core.high_availability.TTPReplicaOptimizer.get_parameter_state_dp_zero_for_ttp',
                    get_parameter_state_dp_zero_with_high_availability_wrapper,
                )
            if args.enable_worker_reboot or args.enable_elastic_training:
                from mindspeed_llm.tasks.high_availability.initialize_patch import (
                ...
                patch_manager.register_patch('torch.distributed.distributed_c10d.new_group', new_group_wrapper)
            if args.enable_elastic_training:
                ...
                patch_manager.register_patch(
                    'megatron.core.timers.Timers._get_elapsed_time_all_ranks', patch_world_size_func_wrapper
                )
                patch_manager.register_patch(
                    'megatron.core.timers.Timers._get_all_ranks_time_string', patch_world_size_func_wrapper
                )
                patch_manager.register_patch('megatron.core.timers.Timers.log', log_wrapper)
                patch_manager.register_patch('megatron.training.utils.is_last_rank', is_last_rank_wrapper)
                patch_manager.register_patch(
                ...
                patch_manager.register_patch('megatron.training.one_logger_utils.track_app_tag', track_app_tag_wrapper)
                patch_manager.register_patch(
            ...
            if getattr(args, 'mtp_num_layers', None) is not None:
                ...
                patch_manager.register_patch(
                    'megatron.core.transformer.multi_token_prediction.MTPLossLoggingHelper.track_mtp_metrics',
                    track_mtp_metrics,
                )
