...
class MLAFeature(MindSpeedFeature):
    ...
    def register_args(self, parser):
        ...
        group.add_argument(
            "--mla-up-proj-tp-overlap", action='store_true', default=False, help='overlap up proj tp comm'
        )
        group.add_argument(
            "--recompute-mla-up-proj", action='store_true', default=False, help='recompute up projection in mla'
        )
        group.add_argument(
            '--mla-swap-core-attn-out', action='store_true', default=False, help='swap core_attn_out only in mla.'
        )
        group.add_argument(
            '--mla-fa-divide-qk',
            action='store_true',
        ...
        group.add_argument(
            '--enable-mla-scale-q-lora', action='store_true', default=False, help='Enable MLA q lora scaling.'
        )
        group.add_argument(
            '--enable-mla-scale-kv-lora', action='store_true', default=False, help='Enable MLA kv lora scaling.'
        )
        group.add_argument(
        ...
        if args.multi_latent_attention:
            if args.kv_lora_rank is None:
                ...
                raise AssertionError(
                    'The parameter qk-pos-emb-head-dim should be set when use multi_head_latent_attention.'
                )
            ...
            if args.recompute_mla_up_proj:
                if not args.mla_up_proj_tp_overlap and tp_size != 1:
                    raise ValueError(
                        '--recompute-mla-up-proj can only be used without '
                        '--mla-up-proj-tp-overlap when tensor model parallel size is 1'
                    )
