...
要启用此功能，用户需通过命令行参数设置目标层为空操作层。例如，原模型共126层，该模型参数为 `--num-layers 126`，即执行实际计算的层有126层。
...
通过实施自定义增加空操作层的策略，预期能够显著减少流水线中的空泡现象，从而优化计算流程并提升系统性能。这不仅有助于加速模型训练过程，还能最大化硬件资源的利用率。
...
> [!NOTE]
>
> 使用“Ascend自定义空操作层”特性增加空层后总层数发生变化，需要根据包含空层的总层数重新调整流水线（虚拟流水线）的配置。
