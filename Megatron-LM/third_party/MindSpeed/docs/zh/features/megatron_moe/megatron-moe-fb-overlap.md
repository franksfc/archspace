...
在DeepSeek V3模型训练中，DeepSeek团队创新性地提出了双向流水跨microbatch间AllToAll(A2A)掩盖，即DualPipe算法。DualPipe从流水线两端同时输入microbatch，同时借鉴ZeroBubble思想进行dw/dx分离，进一步缩减了流水线空泡（bubble）。同时，不同microbatch前反向无依赖关系，DualPipe通过双流编排计算通信，实现1F1B阶段A2A通信全掩盖，具体细节请参见[DeepSeek-V3 Technical Report](https://arxiv.org/abs/2412.19437)。

具体来说，单层内的前向计算和通信按顺序被拆分为Attention(F)、Dispatch(F)、MLP(F)和Combine(F)四个阶段，单层内的反向计算按顺序被拆分为Combine(B)、MLP(B)、Dispatch(B)和Attention(B)，其中Dispatch和Combine分别为MLP计算前后的A2A通信。通过双流编排正向和反向的计算和通信，实现1F1B阶段A2A通信全掩盖。原理如下图。具体细节请参见[profile-data](https://github.com/deepseek-ai/profile-data)。
...
下图是真实DeepSeek-V3-671B模型使用该特性时，单层内的A2A通信掩盖profiling示意图：
![A2A通信掩盖profiling示意图](../../figures/fb_overlap_profile.png)
...
- 当前仅支持`--moe-token-dispatcher-type=alltoall`，暂不支持`allgather/alltoall_seq`Dispatcher。
- 不建议同时使用`--swap-attention`，开启后性能劣化。
- 当前需设置`--expert-tensor-parallel-size=1`，暂不支持专家TP。
- 当前暂不支持Megatron MoE Token Drop&Pad模式，支持Dropless及Drop模式。
- 当前依赖GroupedMatmul，确保打开`--moe-grouped-gemm`。
- 当前仅支持`--moe-zero-memory=level0`，且不支持`moe-zero-memory-num-layers`配置。
- 当前暂不支持异步DP通信掩盖，需关闭`--overlap-grad-reduce`。
- 当前仅支持Mcore Models，不能打开`--use_legacy_models`。
- 在VPP流水下，存在如下额外约束：
    - GBS > 1 \* DP \* PP \* MBS
