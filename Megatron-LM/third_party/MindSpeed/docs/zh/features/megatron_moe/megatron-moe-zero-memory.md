...
支持`alltoall` 与 `alltoall_seq` dispatcher。
...
- 如在`--moe-fb-overlap`条件下使用，具体注意事项请参照[Megatron MoE跨microbatch间A2A通信掩盖](megatron-moe-fb-overlap.md)。

参数说明：

- 对于`--moe-zero-memory level0`，因为性能损失较小，不支持配置层数，功能为所有层使能。

- 对于`--moe-zero-memory level1`，默认为所有层使能，也可配置内存优化的层数：`--moe-zero-memory-num-layers x`，其中x为设置层数，x应大于或等于0且小于等于模型层数(num_layers//pp)。
