...
此外，alltoall分支中，兼容了Megatron的moe-shared-expert-overlap方案，并通过更细粒度的掩盖，可做到较原生方案，性能进一步提升。
...
启动该特性会导致显存占用增加，属正常现象。此时，可搭配ZeroMemory特性调整显存使用状况。ZeroMemory介绍及设置可参考[ZeroMemory文档](megatron-moe-zero-memory.md)。
