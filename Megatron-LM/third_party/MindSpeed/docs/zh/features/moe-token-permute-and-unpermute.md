...
在MoE架构中，MoEAlltoAllTokenDispatcher调度器负责将token分配给各个专家进行处理，并将处理后的结果重新组合回原始的token顺序。这个过程通常涉及到以下步骤：
...
- `--moe-token-dispatcher-type alltoall`时，设置`--expert-tensor-parallel-size 1`
- `--moe-token-dispatcher-type alltoall_seq`时，开启`--moe-tp-extend-ep`
...
3.系统环境要求：仅限于版本标识为`CANN 8.3.RC1`/`TorchNPU 7.2.RC1`及其后续所有迭代版本的系统环境。
