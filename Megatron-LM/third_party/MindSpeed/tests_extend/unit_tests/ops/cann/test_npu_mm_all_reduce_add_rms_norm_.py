...
class TestNPUInplaceMMAllReduceAddRmsNorm(DistributedTest):
    ...
    def supported_op_exec(self, x1, x2, hcomm_info, residual_shape, residual, gamma, epsilon, dequant_scale):
        output_npu = torch_npu.npu_mm_all_reduce_base(
            x1=x1, x2=x2, hcom=hcomm_info, reduce_op="sum", dequant_scale=dequant_scale
        )
        output_npu = output_npu.reshape(residual_shape)
        output_golden, rstd_golden, y_golden = torch_npu.npu_add_rms_norm(output_npu, residual, gamma, epsilon)
        return y_golden, output_golden

    def custom_op_exec(self, x1, x2, hcomm_info, residual, gamma, epsilon, dequant_scale):
        y, norm_out = npu_mm_all_reduce_add_rms_norm_(
            x1=x1, x2=x2, residual=residual, gamma=gamma, hcom=hcomm_info, epsilon=epsilon, dequant_scale=dequant_scale
        )
        return y, norm_out
    ...
    def test_npu_mm_all_reduce_add_rms_norm_(self, dtype):
        ...
        print(
            "current device: {}, local rank = {}, hcomm_info = {}".format(
                torch_npu.npu.current_device(), rank, hcomm_info
            )
        )
        ...
        epsilon = 0.000001
        support_y, support_output = self.supported_op_exec(
            x1_npu, x2_npu, hcomm_info, residual_shape, residual_npu, gamma_npu, epsilon, dequant_scale_npu
        )
