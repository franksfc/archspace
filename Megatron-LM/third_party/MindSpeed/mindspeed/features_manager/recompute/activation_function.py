...
class RecomputeActivationFeature(MindSpeedFeature):
    ...
    def is_need_apply(self, args):
        return super().is_need_apply(args) or (
            self.optimization_level <= args.optimization_level
            and (getattr(args, 'recompute_dense_mlp', False) or getattr(args, 'swap_dense_mlp', False))
        )
    ...
    def register_args(self, parser):
        ...
        group.add_argument(
            ...
            type=int,
            default=None,
            help='Can be used together with "--recompute-method block." and "--recompute-num-layers". ',
        )
    ...
    def validate_args(self, args):
        if args.recompute_activation_function_num_layers is not None:
            ...
            if args.recompute_activation_function_num_layers > args.num_layers:
                raise ValueError(
                    f'--recompute-activation-function-num-layers ({args.recompute_activation_function_num_layers}) '
    ...
    def register_patches(self, patch_manager, args):
        ...
        if (
            ...
            or getattr(args, 'recompute_dense_mlp', False)
            or getattr(args, 'swap_dense_mlp', False)
        ):
            ...
            patch_manager.register_patch(
                'megatron.core.transformer.mlp.MLP.forward', mindspeed_activation_recompute_forward
            )
