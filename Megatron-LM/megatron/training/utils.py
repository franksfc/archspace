...
def calc_params_l2_norm(model, force_create_fp32_copy=False):
    ...
    if ranks_in_dense_reduce_group == ranks_in_expert_reduce_group:
        norm_2 += moe_norm_2
        if len(ranks_in_dense_reduce_group) > 1:
            torch.distributed.all_reduce(
                norm_2,
                op=torch.distributed.ReduceOp.SUM,
    ...
    # If dense and expert reduce groups are different, reduce then sum.
        ...
        if len(ranks_in_expert_reduce_group) > 1:
            torch.distributed.all_reduce(
                moe_norm_2,
                op=torch.distributed.ReduceOp.SUM,
...
def reduce_max_stat_across_model_parallel_group(stat: float) -> float:
    ...
    if torch.distributed.get_world_size(group=model_parallel_group) == 1:
        return stat
...
def get_batch_on_this_tp_rank(data_iterator):
    ...
    def _broadcast(item):
       if item is not None and mpu.get_tensor_model_parallel_world_size() > 1:
