...
def preprocess_common_state_dict(common_state_dict):
    ...
    # Remove rank-local runtime fields from the validation copy.
    for rank_local_key in ('local_rank', 'rank'):
        preprocessed_common_state_dict['args'].pop(rank_local_key, None)
    # Stage-3/SFT intentionally construct the packed Dataset/DataLoader only
    # on CP loader owners, so these derived lengths do not exist on the other
    # CP ranks.  They are not checkpoint state: the original common state dict
    # (including rank-0's informational values) is still returned to and saved
    # by dist-checkpointing.  Keep validating the fields for every backend that
    # does build its dataset on all ranks.
    if preprocessed_common_state_dict['args'].get('dataset_backend') in (
        'olmo3_numpy_packed',
        'olmo3_sft_numpy',
    ):
        preprocessed_common_state_dict['args'].pop('dataset_train_len', None)
        preprocessed_common_state_dict['args'].pop('dataset_valid_len', None)
...
def train_step(forward_step_func, data_iterator,
    ...
    if mpu.is_pipeline_last_stage(ignore_virtual=True):
        deferred_reporters = []
        for microbatch_loss in losses_reduced:
            if len(microbatch_loss) != 1:
                deferred_reporters = []
                break
            reporter = next(iter(microbatch_loss.values()))
            if not callable(getattr(reporter, 'finalize_loss_metrics', None)):
                deferred_reporters = []
                break
            deferred_reporters.append(reporter)
        if deferred_reporters:
            # The reporting all-reduce was launched from the forward loss.
            # Waiting after backward and the optimizer hides its latency while
            # leaving the optimization loss and gradient path untouched.
            reporting = None
            metric_names = None
            for reporter in deferred_reporters:
                values = reporter.finalize_loss_metrics()
                if values.ndim != 1 or values.numel() != 4:
                    raise RuntimeError(
                        'Deferred loss reporting must return '
                        '[total_sum, lm_sum, z_sum, valid_tokens].'
                    )
                names = tuple(reporter.metric_names)
                if metric_names is None:
                    metric_names = names
                elif names != metric_names:
                    raise RuntimeError(
                        'Deferred loss reporters disagree on metric names.'
                    )
                reporting = values if reporting is None else reporting + values
            denominator = reporting[3].clamp_min(1.0)
            loss_reduced = {
                metric_names[index]: reporting[index] / denominator
...
def build_train_valid_test_data_loaders(
    ...
    is_cp_loader_owner = (
        ...
        and (
            not cp_loader_owner_only
            or mpu.get_context_parallel_rank() == 0
        )
    )

    # Construct the data pipeline
    if (is_distributed and not cp_loader_owner_only) or is_cp_loader_owner:
