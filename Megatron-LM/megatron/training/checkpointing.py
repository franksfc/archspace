...
from megatron.core.dist_checkpointing.mapping import CheckpointingException, ShardedObject
...
_OLMO3_STAGE_TRANSITION_RESET_SHARDED_KEYS = frozenset({
    'rng_state',
    'rerun_state_machine_state',
})


def _validate_olmo3_stage_transition_sharded_mismatches(load_result):
    """Accept only the sharded state intentionally reset at a stage boundary.

    OLMo 3 stage transitions deliberately omit RNG and rerun-state templates
    because those states are topology-bound and the new stage starts a fresh
    data/scheduler stream.  ``torch_dist`` still reports the source objects as
    missing under a strict all-key comparison.  Inspect the complete mismatch
    result instead of weakening strictness globally, and keep every model and
    optimizer shard fail-closed.
    """

    if not isinstance(load_result, tuple) or len(load_result) != 3:
        raise CheckpointingException(
            'OLMo3 stage-transition checkpoint load did not return the '
            'state/missing/unexpected strict-validation tuple.'
        )
    state_dict, missing_keys, unexpected_keys = load_result
    missing_keys = set(missing_keys)
    unexpected_keys = set(unexpected_keys)
    disallowed_missing = (
        missing_keys - _OLMO3_STAGE_TRANSITION_RESET_SHARDED_KEYS
    )
    if disallowed_missing or unexpected_keys:
        raise CheckpointingException(
            'OLMo3 stage-transition checkpoint mismatch outside the '
            'intentional RNG/rerun reset contract: '
            f'missing={sorted(missing_keys)}, '
            f'unexpected={sorted(unexpected_keys)}, '
            f'allowed_missing='
            f'{sorted(_OLMO3_STAGE_TRANSITION_RESET_SHARDED_KEYS)}'
        )
    return state_dict
...
def _is_bootstrap_distributed_checkpoint(tracker_filename, iteration):
    """Return whether iteration zero names a real distributed checkpoint.

    A stage-transition reshard writes the new stage's initial model/optimizer
    state at ``iter_0000000``.  Zero is therefore valid only when the tracker
    sits next to an actual distributed-checkpoint metadata contract; legacy
    checkpoints and bare/corrupt tracker files retain the historical
    positive-iteration requirement.
    """

    if iteration != 0:
        return False
    checkpoint_root = os.path.dirname(os.path.abspath(tracker_filename))
    ...
    return dist_checkpointing.check_is_distributed_checkpoint(checkpoint_name)
...
def read_metadata(tracker_filename):
    ...
    assert (
        iteration > 0
        or release
        or _is_bootstrap_distributed_checkpoint(tracker_filename, iteration)
...
def _load_global_dist_base_checkpoint(
    ...
    olmo3_stage_transition = bool(
        getattr(args, 'olmo3_stage_transition', False)
    )
    load_strictness = (
        'return_all'
        if olmo3_stage_transition
        else args.dist_ckpt_strictness
    )
    state_dict = dist_checkpointing.load(
        sharded_state_dict,
        checkpoint_name,
        load_strategy,
        strict=load_strictness,
    )
    if olmo3_stage_transition:
        state_dict = _validate_olmo3_stage_transition_sharded_mismatches(
            state_dict
        )
    return state_dict, checkpoint_name, release, CheckpointType.GLOBAL
...
def _capture_olmo3_stage_optimizer_schedule(optimizer):
    """Capture the new stage's optimizer hyperparameters before loading moments."""
    if optimizer is None:
        return None
    return [
        {key: group[key] for key in _OLMO3_STAGE_SCHEDULE_KEYS if key in group}
        for group in optimizer.param_groups
    ]


def _restore_olmo3_stage_optimizer_schedule(optimizer, stage_schedule):
    """Keep loaded per-parameter Adam state but restore the new stage's schedule."""
    if optimizer is None or stage_schedule is None:
        raise RuntimeError('OLMo3 stage transition requires an optimizer schedule.')
    if len(stage_schedule) != len(optimizer.param_groups):
        raise RuntimeError(
            'OLMo3 stage transition changed optimizer parameter-group structure.'
        )
    for group, schedule_values in zip(optimizer.param_groups, stage_schedule):
        group.update(schedule_values)
...
def load_checkpoint(ddp_model, optimizer, opt_param_scheduler, load_arg='load', strict=True,
    ...
    """
    args = get_args()
    stage_transition = bool(getattr(args, 'olmo3_stage_transition', False))
    if stage_transition:
        if args.finetune:
            raise ValueError('--olmo3-stage-transition cannot be combined with --finetune.')
        if args.no_load_optim or optimizer is None:
            raise ValueError(
                '--olmo3-stage-transition requires loading the previous-stage optimizer state.'
            )
    stage_param_group_schedule = (
        _capture_olmo3_stage_optimizer_schedule(optimizer)
        if stage_transition
        else None
    )
    load_dir = getattr(args, load_arg)
    ...
    if ckpt_format == "torch_dist":
        ...
        if (ckpt_tp_pp == run_tp_pp and not release and not args.finetune
                and not stage_transition and not args.no_load_rng
        ...
        if (not release and not args.finetune and not args.no_load_optim
            ...
            gen_sd_opt_param_scheduler = (
                None if stage_transition else opt_param_scheduler
            )
    ...
    # Set iteration.
    if args.finetune or release or stage_transition:
        iteration = 0
    else:
        try:
    ...
    if 'args' in state_dict and not args.finetune and not stage_transition:
        checkpoint_args = state_dict['args']
        ...
        print_rank_0(
            'OLMo3 stage transition intentionally keeps the new stage runtime arguments.'
        )
    else:
    ...
    if not release and not args.finetune and not args.no_load_optim:
        try:
            ...
            # Load scheduler.
            if opt_param_scheduler is not None and not stage_transition:
    ...
    if stage_transition:
        iteration = 0
        num_floating_point_operations_so_far = 0
        _restore_olmo3_stage_optimizer_schedule(
            optimizer, stage_param_group_schedule
        )
        if opt_param_scheduler is not None:
            opt_param_scheduler.num_steps = 0
            opt_param_scheduler.step(0)
        print_rank_0(
            'OLMo3 stage transition loaded model + Adam moments and reset '
            'iteration, sample/RNG/rerun/scheduler state.'
        )
    else:
        # rerun state
        try:
    ...
    # rng states.
    if not release and not args.finetune and not stage_transition and not args.no_load_rng:
        try:
