...
def _uses_olmo3_weight_decay(model_chunk: MegatronModule) -> bool:
    """Whether a model chunk explicitly requests the OLMo3 decay policy."""

    config = getattr(model_chunk, 'config', None)
    if config is None:
        config = getattr(getattr(model_chunk, 'module', None), 'config', None)
    configured = getattr(config, 'olmo3_weight_decay', None)
    if configured is not None:
        return bool(configured)
    return _parse_env_flag('MEGATRON_OLMO3_WEIGHT_DECAY')


def _is_olmo3_input_embedding(name: str, param: torch.nn.Parameter) -> bool:
    """Identify the input embedding without matching an untied output head."""

    explicit_marker = getattr(param, '_olmo3_input_embedding', None)
    if explicit_marker is not None:
        return bool(explicit_marker)
    return name.endswith('embedding.word_embeddings.weight')
...
def _get_param_groups(
    ...
    for model_chunk in model_chunks:
        use_olmo3_weight_decay = _uses_olmo3_weight_decay(model_chunk)
        ...
        for name, param in named_parameters:
            ...
            if use_olmo3_weight_decay:
                # OLMo3 decays every trainable parameter except the input
                # token embedding, including norms, biases, learned scales and
                # the untied output projection.
                no_wd = _is_olmo3_input_embedding(name, param)
        ...
        if config.use_distributed_optimizer:
            ...
            if intra_dist_opt_group is not None:
                if model_parallel_group is None:
                    raise RuntimeError(
                        "Partial distributed optimizer requires a "
                        "model-parallel group for gradient statistics"
                    )
...
def get_megatron_optimizer(
    ...
    dense_intra_dist_opt_group = None
    if ddp_config.num_distributed_optimizer_instances > 1:
        dense_intra_dist_opt_group = mpu.get_data_parallel_group(
    ...
    for dense_model_chunks, overlap_param_gather_with_optimizer_step in zip(
        ...
        optimizers.append(
            _get_megatron_optimizer_based_on_param_groups(
                ...
                data_parallel_group_gloo=data_parallel_group_gloo,
                data_parallel_group_idx=model_parallel_rank,
                intra_dist_opt_group=dense_intra_dist_opt_group,
                distributed_optimizer_instance_id=distributed_optimizer_instance_id,
            )
        )
        model_chunk_offset += 1
