...
class LanguageModule(MegatronModule):
    ...
    def compute_language_model_loss_components(
        self, labels: Tensor, logits: Tensor
    ) -> tuple[Tensor, Tensor, Tensor]:
        """Compute total, language-modeling, and scaled z-loss per token.
        ...
        Returns:
            tuple[Tensor, Tensor, Tensor]: Total optimization loss, pure language-modeling loss,
            and scaled z-loss. Every tensor has dimensions [batch size, sequence_length], but only
            the total optimization loss is differentiable.
        """
        # [b s] => [s b]
        labels = labels.transpose(0, 1).contiguous()
        z_loss_coeff = getattr(self.config, 'vocab_z_loss_coeff', 0.0)
        if not math.isfinite(z_loss_coeff) or z_loss_coeff < 0.0:
            raise ValueError('vocab_z_loss_coeff must be finite and non-negative')
        if z_loss_coeff > 0.0 and self.config.cross_entropy_loss_fusion:
            raise ValueError(
                'Dense vocabulary z-loss is not implemented by fused cross entropy; '
                'disable cross_entropy_loss_fusion when vocab_z_loss_coeff is positive.'
            )
        ...
        if self.config.cross_entropy_loss_fusion:
            ...
            loss, lm_loss, z_loss = (
                tensor_parallel.vocab_parallel_cross_entropy_with_loss_components(
                    logits, labels, z_loss_coeff=z_loss_coeff
                )
            )
        ...
        z_loss = z_loss.transpose(0, 1).contiguous()
        return loss, lm_loss, z_loss

    def compute_language_model_loss(self, labels: Tensor, logits: Tensor) -> Tensor:
        """Compute cross entropy across the vocabulary, including configured z-loss."""
        loss, _, _ = LanguageModule.compute_language_model_loss_components(self, labels, logits)
        return loss
    ...
    def setup_embeddings_and_output_layer(self) -> None:
        ...
        if self.pre_process:
            ...
            # This marker distinguishes the input table from an untied output
            # projection for OLMo3's optimizer grouping policy.
            self.embedding.word_embeddings.weight._olmo3_input_embedding = True
