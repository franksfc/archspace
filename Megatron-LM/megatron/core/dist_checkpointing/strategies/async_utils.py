    ...
    def execute_sync(self) -> None:
        ...
        if self.async_fn is not None:
            ...
            if self.preload_fn is not None:
                # Match TemporalAsyncCaller's staging semantics, but keep both
                # staging and writing in the training process.  This is required
                # by the NPU no-fork synchronous checkpoint path.
...
class AsyncCallsQueue:
    ...
    def schedule_async_request(self, async_request: AsyncRequest) -> int:
        ...
        """
        self.call_idx += 1
        if os.environ.get("MCORE_DIST_CKPT_NO_FORK", "0") == "1":
            # Honor the no-fork contract even if the caller requested an
            # asynchronous checkpoint.  FileSystemWriterAsync uses a
            # process-local results queue in this mode, so the request and its
            # finalizers must run synchronously in the training process.
            if len(async_request._fields) != len(AsyncRequest._fields):
                async_request = AsyncRequest(**async_request._asdict())
            async_request._replace(call_idx=self.call_idx).execute_sync()
