...
class DistributedDataParallel(_BaseDataParallel):
    ...
    def __init__(
        ...
        def _allocate_buffers_for_parameters(
            ...
            if self.ddp_config.num_distributed_optimizer_instances > 1:
                ...
                if tp_lane_pack == 2 and bucket_groups:
                    if not self.ddp_config.overlap_grad_reduce:
                        raise RuntimeError(
                            "OLMo3 HSDP TP-lane pack2 requires overlap_grad_reduce"
                        )
                    if not olmo3_hsdp_inter_overlap_backward_enabled():
                        raise RuntimeError(
                            "OLMo3 HSDP TP-lane pack2 requires "
                            "OLMO3_HSDP_INTER_OVERLAP_BACKWARD=1"
                        )
                    if any(len(group.buckets) != 1 for group in bucket_groups):
                        raise RuntimeError(
                    ...
                    if len(tp_members) != 2:
                        raise RuntimeError(
                            "OLMo3 HSDP TP-lane pack2 requires tensor parallel size 2"
                        )
                    tp_rank = torch.distributed.get_rank(group=tp_group)
                    local_world_size_raw = os.getenv("LOCAL_WORLD_SIZE", "").strip()
                    try:
                        local_world_size = int(local_world_size_raw)
                    except ValueError as exc:
                        raise RuntimeError(
                            "OLMo3 HSDP TP-lane pack2 requires integer "
                            f"LOCAL_WORLD_SIZE, got {local_world_size_raw!r}"
                        ) from exc
                    if local_world_size < 2:
                        raise RuntimeError(
                            "OLMo3 HSDP TP-lane pack2 requires LOCAL_WORLD_SIZE >= 2"
                        )
                    node_index = torch.distributed.get_rank() // local_world_size
                    if (
                        any(
                            member // local_world_size != node_index
                            for member in tp_members
                        )
                        or tp_members[1] != tp_members[0] + 1
                        or tp_members[0] % 2
                    ):
                        raise RuntimeError(
                            "OLMo3 HSDP TP-lane pack2 requires an adjacent "
                            "even/odd TP pair inside one physical node"
                        )

                    intra_size = bucket_groups[
                        0
                    ].intra_distributed_optimizer_instance_size
                    intra_rank = bucket_groups[
                        0
                    ].intra_distributed_optimizer_instance_rank
                    if any(
                        group.intra_distributed_optimizer_instance_size != intra_size
                        or group.intra_distributed_optimizer_instance_rank != intra_rank
                        for group in bucket_groups[1:]
                    ):
                        raise RuntimeError(
                            "OLMo3 HSDP TP-lane pack2 requires one shared "
                            "optimizer-shard topology"
                        )
                    first_grad = bucket_groups[0].buckets[0].grad_data
                    if any(
                        bucket.grad_data.dtype != first_grad.dtype
                        or bucket.grad_data.device != first_grad.device
                        for group in bucket_groups
                        for bucket in group.buckets
                    ):
                        raise RuntimeError(
                            "OLMo3 HSDP TP-lane pack2 requires one gradient dtype and device"
                        )
                    maximum_shard_elements = max(
                        bucket.grad_data.numel() // intra_size
                        for group in bucket_groups
                        for bucket in group.buckets
                    )
                    shared_buffer = (
                        torch.empty(
                            tp_lane_pack * maximum_shard_elements,
                    ...
                    for index, bucket_group in enumerate(bucket_groups):
                        bucket_group.olmo3_tp_lane_pack_group = tp_group
                        bucket_group.olmo3_tp_lane_pack_buffer = shared_buffer
                        bucket_group.olmo3_tp_lane_pack_tag = 6100 + 2 * index
    ...
    def finish_grad_sync(self):
        ...
        if (
            ...
            # Dispatch every dependent inter-instance AR before waiting on any
            # one bucket. This avoids an RS -> AR -> wait serialization tail.
            for bucket_group in bucket_groups:
                if (
                    bucket_group.intra_distributed_optimizer_instance_size > 1
                    and bucket_group.grad_reduce_handle is None
                ):
                    bucket_group.start_inter_grad_sync()
        for bucket_group in bucket_groups:
