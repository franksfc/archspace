...
def _apply_packed_document_rotary_pos_emb(
    tensor: Tensor,
    freqs: Tensor,
    ...
    *,
    ...
    """Apply document-relative RoPE to CP-local packed Q/K before Ulysses.

    MCore's generic THD helper derives positions by dividing document lengths
    across CP ranks.  OLMo3 permits arbitrary document boundaries, so its
    dataloader supplies the exact reset position of every local token.  This
    helper deliberately indexes by those positions and then presents the NPU
    fused RoPE kernel with its required 4-D ABI.
    """

    if packed_position_ids.ndim != 2:
        raise ValueError(
            "Packed OLMo3 position_ids must have "
            "[micro_batch, local_sequence] shape; "
            f"got {tuple(packed_position_ids.shape)}."
        )
    if freqs.ndim < 2 or freqs.shape[0] < 1:
        raise ValueError(
            f"Packed OLMo3 rotary table has invalid shape {tuple(freqs.shape)}."
        )
    frequency_table = freqs.reshape(
        freqs.shape[0],
        -1,
        freqs.shape[-1],
    )
    if frequency_table.shape[1] != 1:
        raise ValueError(
            "Packed OLMo3 rotary frequencies must have only singleton "
            f"middle dimensions, got {tuple(freqs.shape)}."
        )
    frequency_table = frequency_table[:, 0, :]

    restore_thd = tensor.ndim == 3
    if tensor.ndim == 4:
        sequence_length, batch_size = tensor.shape[:2]
        expected_position_shape = (batch_size, sequence_length)
        if tuple(packed_position_ids.shape) != expected_position_shape:
            raise ValueError(
                "Packed OLMo3 position_ids must match SBHD batch/sequence; "
                f"got {tuple(packed_position_ids.shape)}, expected "
                f"{expected_position_shape}."
            )
        positions = (
            packed_position_ids.transpose(0, 1)
            .contiguous()
            .reshape(-1)
            .to(device=freqs.device, dtype=torch.long)
        )
    elif tensor.ndim == 3:
        positions = packed_position_ids.reshape(-1).to(
            device=freqs.device,
            dtype=torch.long,
        )
        if positions.numel() != tensor.shape[0]:
            raise ValueError(
                "Packed OLMo3 THD position_ids must contain one entry "
                f"per token; got {positions.numel()} for {tensor.shape[0]}."
            )
    else:
        raise ValueError(
            "Packed OLMo3 RoPE expects THD or SBHD Q/K tensors, "
            f"got shape {tuple(tensor.shape)}."
        )
    if positions.numel() and (
        int(positions.min().item()) < 0
        or int(positions.max().item()) >= frequency_table.shape[0]
    ):
        raise ValueError(
            "Packed OLMo3 position_ids exceed the rotary table: "
            f"range=[{int(positions.min().item())}, "
            f"{int(positions.max().item())}], "
            f"table={frequency_table.shape[0]}."
        )
    local_freqs = frequency_table.index_select(0, positions)
    if restore_thd:
        tensor = tensor.unsqueeze(1)
        local_freqs = local_freqs.unsqueeze(1).unsqueeze(1)
    else:
    ...
    if local_freqs.ndim != 4:
        raise ValueError(
            "Packed OLMo3 rotary frequencies must be broadcastable "
            f"to SBHD, got shape {tuple(local_freqs.shape)}."
        )
    rotated = _apply_rotary_pos_emb_bshd(
        tensor,
        local_freqs,
    ...
    return rotated.squeeze(1) if restore_thd else rotated
...
class Attention(MegatronModule, ABC):
        ...
        if self.config.flash_decode:
            ...
            if rotary_pos_sin_q is not None and rotary_pos_sin_k is not None:
                key = apply_rotary_pos_emb_with_cos_sin(
                    ...
                    full_precision=self.config.rope_full_precision,
                )
    ...
    def forward(
        ...
        *,
        ...
        # OLMo3 Stage 3 selects a layer-local RoPE table. Full-attention
        # layers additionally carry YaRN's per-Q/K m-scale. Keep this small
        # protocol generic so Megatron Core does not import model code.
        rotary_mscale = 1.0
        if getattr(rotary_pos_emb, "_olmo3_rotary_output", False):
            rotary_mscale = float(rotary_pos_emb.mscale)
            rotary_pos_emb = rotary_pos_emb.freqs
        ...
        # The OLMo3 MindSpeed adapter owns both document-aware TND conversion
        # and optional Ulysses communication. Keep its packed Q/K/V in SBHD
        # regardless of CP size; all other backends retain MCore's THD input.
        packed_backend_keeps_sbhd = (
            packed_seq_params is not None
            and bool(getattr(self.core_attention, "olmo3_keeps_packed_sbhd", False))
        )
        if packed_seq_params is not None and not packed_backend_keeps_sbhd:
        ...
        if rotary_pos_emb is not None and not self.config.flash_decode:
            ...
            packed_position_ids = (
                getattr(packed_seq_params, "position_ids", None)
                if packed_seq_params is not None
                else None
            )
            ...
            if q_pos_emb is not None:
                ...
                if packed_position_ids is not None:
                    query = _apply_packed_document_rotary_pos_emb(
                        query,
                        q_pos_emb,
                        packed_position_ids,
                        self.config,
                        mscale=rotary_mscale,
                    )
                elif rotary_mscale != 1.0:
                    # Fused RoPE APIs in this MCore release silently ignore
                    # YaRN m-scale, so Full-attention Stage-3 layers must use
                    # the exact unfused equation.
            ...
            if k_pos_emb is not None and packed_position_ids is not None:
                key = _apply_packed_document_rotary_pos_emb(
                    key,
                    k_pos_emb,
                    packed_position_ids,
                    self.config,
            ...
            elif k_pos_emb is not None and rotary_mscale != 1.0:
        ...
        if packed_seq_params is not None and packed_seq_params.qkv_format == 'thd':
            if packed_backend_keeps_sbhd:
                if core_attn_out.ndim != 3:
                    raise ValueError(
                        "The OLMo3 packed backend must restore [sequence, batch, hidden] "
                        f"before the output projection, got {tuple(core_attn_out.shape)}."
                    )
            else:
                # Stock packed backends expose THD and retain MCore's historical
                # singleton dummy batch contract.
