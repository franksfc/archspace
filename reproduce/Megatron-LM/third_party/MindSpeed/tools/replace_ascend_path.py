...
"""
Batch-replace the Ascend driver install path in the MindSpeed repository.
...
Usage:
  python3 tools/replace_ascend_path.py [options]
...
def process_file(filepath, source_path, target_path, dry_run):
    ...
    if not dry_run:
        try:
            ...
            print(f"[SKIP] Cannot write {filepath}: {e}", file=sys.stderr)
            return False, 0
...
def main():
    parser = argparse.ArgumentParser(
        description='Batch-replace the Ascend driver install path in the MindSpeed repository',
