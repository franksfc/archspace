...
* 开启利用bubble进行重计算功能：`--recompute-in-bubble`，另外必须开启虚拟流水线并行特性，使用此功能前不能开启重计算，recompute_num_layers参数需为None或0。

* 开启提前重计算以及去除不必要重计算功能：`--recompute-in-advance`，另外必须开启虚拟流水线并行特性，使用此功能前需要开启重计算，且不支持recompute_method为uniform，recompute_num_layers不能为None或0。

> [!NOTE]
>
> * --recompute-in-bubble特性暂不兼容完全重计算uniform、完全重计算block、选择重计算、自适应选择重计算、swap-attention、no-align-grad-reduce和no-overlap-p2p-communication特性，不兼容moe场景下的--moe-adaptive-recompute-activation、--moe-layer-recompute特性。
> * --recompute-in-advance特性暂不兼容完全重计算uniform、选择重计算、自适应选择重计算、no-align-grad-reduce和no-overlap-p2p-communication特性。
