...
其中，$ELP_t$为$t$步的专家负载预测，$EL_t$为采集的$t$步专家负载数据，$θ$为EMA权重，一般取0.9-0.999。$θ$越大，负载预测越关注历史数据，当前负载数据影响越小。本方案中取θ=0.9，相比更大的θ值，当前负载数据的权重更高，更能捕捉到当前的负载特征，基于此的专家迁移对变化负载的适应性更强。
...
1. 初始化，每个 device 上的专家数量为 0，处理的 token 数量为 0

2. 根据专家负载预测结果，在专家集合中找到负载最大的专家

3. 遍历每一个 device，找到专家数量小于预设值且处理 token 数量最少的 device

4. 将负载最大的专家分配给上一步中的 device，并将该专家从专家集合中剔除

5. 重复第 2 步，直至所有专家分配完成。
...
支持参数副本复用（reuse_fp32_param）和分布式优化器（use_distributed_optimizer）以及tp_extend_ep功能。
