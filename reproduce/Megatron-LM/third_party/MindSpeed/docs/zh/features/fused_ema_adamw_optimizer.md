...
```model_params```为模型参数，```ema_decay```为超参数。其中，```ema_decay```可在训练脚本中使用`--ema-decay <value>`来指定，若脚本中未指定，则默认ema_decay为0.9999。<br>
...
2. 在脚本中添加`--ema-decay <value>`，可指定ema_decay，如未指定，则默认为0.9999。<br>
