...
Build MindSpeed CI Docker Image (includes MindSpeed-LLM, vllm, vllm-ascend, verl for ST tests)

Options:
    -t, --npu-type TYPE          NPU type: a3 or 910b (default: 910b)
    -o, --os OS                  OS: openeuler24.03 or ubuntu22.04 (default: openeuler24.03)
    -i, --image-name NAME        Image name (default: mindspeed-ci:{branch}-{chip}-{os}-py{py_ver}-{arch})
    -n, --no-cache               Build without cache
    --base-image-version VER     Base image CANN version (default: 9.0.1)
    --base-image IMAGE           Full base image name (higher priority than --base-image-version)
    --python-version VER         Python tag in the CANN base image (default: 3.12)
    --torch-version VER          PyTorch version (default: 2.9.0)
    --torch-npu-version VER      torch_npu version (default: 2.9.0)
    --mindspeed-branch REF       MindSpeed branch (default: master)
    --megatron-branch REF        Megatron-LM branch (default: core_v0.12.1)
    --mindspeed-llm-branch REF   MindSpeed-LLM branch (default: master)
    --vllm-version VER           vLLM version (default: v0.18.0)
    --vllm-ascend-version VER    vLLM-ascend version (default: releases/v0.18.0)
    --verl-version VER           verl version (default: v0.7.0)
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --vllm-ascend-version)    VLLM_ASCEND_VERSION="$2"; shift 2 ;;
        --verl-version)           VERL_VERSION="$2"; shift 2 ;;
...
if [ -z "$IMAGE_NAME" ]; then
    ...
    IMAGE_NAME="mindspeed-ci:${TAG_REF}-${NPU_TYPE_LOWER}-${OS}-py${PYTHON_VERSION}-${ARCH_NAME}"
fi

cd "$SCRIPT_DIR"

# Prepare configure_repo.sh for Docker build context
ORIG_REPO_SCRIPT="${SCRIPT_DIR}/configure_repo.sh"
if [ -f "$ORIG_REPO_SCRIPT" ]; then
    cp "$ORIG_REPO_SCRIPT" "${ORIG_REPO_SCRIPT}.bak"
fi
cp "${SCRIPT_DIR}/${REPO_SCRIPT}" configure_repo.sh
trap 'rm -f configure_repo.sh; [ -f "${ORIG_REPO_SCRIPT}.bak" ] && mv "${ORIG_REPO_SCRIPT}.bak" "${ORIG_REPO_SCRIPT}"' EXIT
...
BUILD_ARGS="$BUILD_ARGS --build-arg VLLM_VERSION=${VLLM_VERSION}"
BUILD_ARGS="$BUILD_ARGS --build-arg VLLM_ASCEND_VERSION=${VLLM_ASCEND_VERSION}"
BUILD_ARGS="$BUILD_ARGS --build-arg VERL_VERSION=${VERL_VERSION}"
...
echo "=========================================="
echo "MindSpeed CI Docker Build Configuration"
echo "=========================================="
...
echo "MindSpeed-LLM Ref:   ${MINDSPEED_LLM_BRANCH}"
echo "vLLM Version:        ${VLLM_VERSION}"
echo "vLLM-ascend Version: ${VLLM_ASCEND_VERSION}"
echo "verl Version:        ${VERL_VERSION}"
