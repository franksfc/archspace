...
def setup_model_and_optimizer(seed, bf16=True):
    ...
    config = OptimizerConfig(
        lr=1e-4, bf16=bf16, params_dtype=torch.bfloat16 if bf16 else torch.float, use_distributed_optimizer=bf16
    )
...
class TestGatherAndScatter(DistributedTest):
    ...
    def test_scatter(self, tp_pp):
        ...
        if data_parallel_rank == 0:
            send_tensors = [
                torch.rand(recv_shape, dtype=torch.float32, device="cpu") for _ in range(data_parallel_world_size)
            ]
        else:
            send_tensors = None
        ...
        _scatter_hccl(recv_tensor_hccl, send_tensors, data_parallel_global_ranks[0], data_parallel_group)
    ...
    def test_gather(self, tp_pp):
        ...
        if data_parallel_rank == 0:
            recv_tensors_gloo = [
                torch.empty(send_shape, dtype=torch.float32, device="cpu") for _ in range(data_parallel_world_size)
            ]
        else:
        ...
        # HCCL Slice Communication Optimization
        recv_tensors_hccl = [torch.empty(send_shape, dtype=torch.float32) for _ in range(data_parallel_world_size)]
        _gather_hccl(
            send_tensor,
            recv_tensors_hccl,
            data_parallel_group,
...
class TestDistributedOptimizer(DistributedTest):
    ...
    def test_full_dp_sharding(self, tp_pp):
        ...
        if args.reuse_fp32_param:
            optimizer.chained_optimizers[0].get_parameter_state_dp_zero_func = types.MethodType(
                get_parameter_state_dp_zero_hccl, optimizer.chained_optimizers[0]
            )
            optimizer.chained_optimizers[0].load_parameter_state_from_dp_zero_func = types.MethodType(
                load_parameter_state_from_dp_zero_hccl, optimizer.chained_optimizers[0]
            )
        else:
            optimizer.chained_optimizers[0].get_parameter_state_dp_zero = types.MethodType(
                get_parameter_state_dp_zero_hccl, optimizer.chained_optimizers[0]
            )
            optimizer.chained_optimizers[0].load_parameter_state_from_dp_zero = types.MethodType(
                load_parameter_state_from_dp_zero_hccl, optimizer.chained_optimizers[0]
            )
        initialize_model_parallel_comm(*tp_pp)
        ...
        optimizer.chained_optimizers[0].load_parameter_state_from_dp_zero(
            optim_param_state_new, update_legacy_format=False
        )
