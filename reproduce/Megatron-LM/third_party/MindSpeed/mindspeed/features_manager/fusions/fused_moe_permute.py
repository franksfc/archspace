...
class FusedMoEPermuteFeature(MindSpeedFeature):
    ...
    def is_need_apply(self, args):
        return self.optimization_level <= args.optimization_level and (
            getattr(args, self.feature_name, None) or getattr(args, "use_fused_moe_token_permute_and_unpermute", None)
        )
    ...
    def validate_args(self, args: Namespace):
        ...
        if args.moe_permute_fusion and args.moe_token_dispatcher_type == 'allgather':  # nosec B105
    ...
    def pre_register_patches(self, patch_manager, args):
        ...
        patch_manager.register_patch(
            'transformer_engine.pytorch.permutation.moe_permute_with_probs', torch.nn.Module, create_dummy=True
        )
        patch_manager.register_patch(
            'transformer_engine.pytorch.permutation.moe_sort_chunks_by_index', torch.nn.Module, create_dummy=True
        )
        patch_manager.register_patch(
        ...
        patch_manager.register_patch(
            'transformer_engine.pytorch.permutation.moe_unpermute', torch.nn.Module, create_dummy=True
        )
    ...
    def register_patches(self, patch_manager, args: Namespace):
        if getattr(args, self.feature_name, None) or getattr(args, "use_fused_moe_token_permute_and_unpermute", None):
            ...
            try:
                ...
                patch_manager.register_patch(
                    'megatron.core.extensions.transformer_engine.fused_sort_chunks_by_index', moe_sort_chunks_by_index
                )
                patch_manager.register_patch(
                    'megatron.core.extensions.transformer_engine.fused_sort_chunks_by_index_with_probs',
                    moe_sort_chunks_by_index_with_probs,
                )
            except ImportError:
                warnings.warn("Currently, fused_sort_chunks_by_index is not supported")
                patch_manager.register_patch(
                    'megatron.core.transformer.moe.moe_utils.sort_chunks_by_idxs', sort_chunks_by_idxs_wrapper
                )
            ...
            if getattr(args, "moe_token_dispatcher_type", None) == "alltoall":
                from mindspeed.core.fusions.fused_moe_permute import preprocess_sync_wrapper
