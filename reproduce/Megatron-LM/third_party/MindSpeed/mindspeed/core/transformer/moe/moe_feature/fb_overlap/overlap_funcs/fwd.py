    ...
    if getattr(args, 'enable_mhc', False):
        ...
        if recomp_norm:
            ...
            pre_mlp_layernorm_output = self.norm_ckpt2.checkpoint(
                ...
                False,
                detached_mlp_mhc_pre_output,  # pylint: disable=possibly-used-before-assignment
            )
        else:
...
def transformer_layer_forward_dense(
    ...
    args = get_full_args()
    recomp_norm = getattr(args, 'recompute_norm', False)
    recomp_dense_mlp = getattr(args, 'recompute_dense_mlp', False) and self.training and torch.is_grad_enabled()
    swap_dense_mlp = getattr(args, 'swap_dense_mlp', False) and self.training and torch.is_grad_enabled()
    ...
    # MLP.
    detached_mlp_input = detach_tensor(pre_mlp_layernorm_output, checkpoint_forward=checkpoint)
    dense_fc1_swap_manager = None
    if recomp_dense_mlp or swap_dense_mlp:
        dense_fc1_memory_context = {
            'mode': 'recompute' if recomp_dense_mlp else 'swap',
            'norm_checkpoint_manager': self.norm_ckpt2 if recomp_norm else None,
        }
        if swap_dense_mlp:
            dense_fc1_memory_context['swap_manager_factory'] = TensorSwapManager

        setattr(self.mlp, DENSE_FC1_MEMORY_CONTEXT, dense_fc1_memory_context)
        try:
            mlp_output_with_bias = self.mlp(detached_mlp_input)
        finally:
            delattr(self.mlp, DENSE_FC1_MEMORY_CONTEXT)
        dense_fc1_swap_manager = dense_fc1_memory_context.get('fc1_swap_manager')
    else:
        mlp_output_with_bias = self.mlp(detached_mlp_input)

    if recomp_norm:
        self.norm_ckpt2.discard_output()
        if not recomp_dense_mlp and mlp_output_with_bias[0].requires_grad:
    ...
    if dense_fc1_swap_manager is not None:
        dense_fc1_swap_manager.wait_swap_out()
    ...
    graph.dense_fc1_swap_manager = dense_fc1_swap_manager
    if hasattr(self.self_attention, 'swap_managers'):
        graph.attn_swap_managers = self.self_attention.swap_managers
...
class IdentityOPFunc(torch.autograd.Function):
    @staticmethod
    def forward(cls, x):  # pylint: disable=bad-staticmethod-argument
        return x

    @staticmethod
    def backward(cls, grad):  # pylint: disable=bad-staticmethod-argument
        return grad
