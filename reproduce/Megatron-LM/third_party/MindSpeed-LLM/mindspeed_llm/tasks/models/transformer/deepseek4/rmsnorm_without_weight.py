...
from mindspeed_llm.tasks.models.transformer.deepseek4.rmsnorm_without_weight_triton_kernel import (
    rmsnorm_without_weight_backward,
)
...
if TRITON_AVAILABLE:
    ...
    class RMSNormWithoutWeightFunction(torch.autograd.Function):
        ...
        def forward(ctx, x: torch.Tensor, norm_eps: float = 1e-6):
            res = rmsnorm_without_weight(x, norm_eps)
        ...
        def backward(
            ...
            grad_x = rmsnorm_without_weight_backward(grad_res, x, res, norm_eps)
...
def rmsnorm_without_weight_triton(x: torch.Tensor, norm_eps: float = 1e-6) -> torch.Tensor:
    res = RMSNormWithoutWeightFunction.apply(x, norm_eps)  # pylint:disable=possibly-used-before-assignment
    return res
