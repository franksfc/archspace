...
from mindspeed_llm.tasks.models.transformer.qwen3_next_full_attention import (
    ...
    CustomQwen3NextSelfAttention,
)
from mindspeed_llm.tasks.models.transformer.qwen3_next_gated_deltanet_attention import (
    CustomQwen3NextGatedDeltaNetAttentionSubmodules,
    CustomQwen3NextGatedDeltaNetAttention,
)

args = get_args()
num_experts, moe_grouped_gemm, qk_layernorm, shared_expert_gate = (
    args.num_experts,
    args.moe_grouped_gemm,
    args.qk_layernorm,
    args.shared_expert_gate,
)
...
# Build the MLP spec dynamically so the same layer spec serves both the
# MoE config (num_experts > 0) and the dense config (no MoE args passed,
# i.e. num_experts unset/0). The MoE branch is unchanged from upstream.
if num_experts:
    # experts spec
    if moe_grouped_gemm:
    ...
    mlp_spec = ModuleSpec(
        ...
        submodules=MoESubmodules(
            ...
            shared_experts=ModuleSpec(
                module=SharedExpertMLP, params={"gate": shared_expert_gate}, submodules=qwen3_next_mlp
            ),
        ),
    )
else:
    # dense spec: standard MLP
    mlp_spec = ModuleSpec(
        module=MLP,
        submodules=qwen3_next_mlp,
    )

linear_attention_spec = ModuleSpec(
    module=CustomQwen3NextGatedDeltaNetAttention,
...
full_attention_spec = ModuleSpec(
