...
from mindspeed.core.transformer.moe.moe_feature import permute, sort_chunks_by_idxs, parallel_state
        ...
        if ctx.use_gmm:
            if config.gemm_gradient_accumulation_fusion:
                npu_groupmatmul_add_fp32(
                    mm2_inputs, grad_mm2_b_inputs * ctx.scaling, group_list, original_weight2_a.main_grad
                )
...
def lora_parallel_grouped_mlp_with_comp_and_comm_overlap_all2all_seq(
    inputs, weights1_a, weights1_b, weights2_a, weight2_b, args, ctx
):
    return LoraParallelGroupedMlpWithCompAndCommOverlapAll2AllSEQ.apply(
        inputs, weights1_a, weights1_b, weights2_a, weight2_b, args, ctx
    )
...
class LoraParallelGroupedMLP(GroupedMLP):
    def __init__(self, num_local_experts, config, lora_config):
        ...
        weight1_a = torch.zeros(
            (self.num_local_experts, self.config.hidden_size, self.lora_r),
            dtype=self.weight1_lora_a.dtype,
            device=self.weight1_lora_a.device,
        )
        weight2_a = torch.zeros(
            (self.num_local_experts, fc2_input_size_per_partition // self.num_local_experts, self.lora_r),
            dtype=self.weight2_lora_a.dtype,
            device=self.weight2_lora_a.device,
        )
    ...
    def forward(
        ...
        if args.moe_alltoall_overlap_comm:
            ...
            return lora_parallel_grouped_mlp_with_comp_and_comm_overlap_all2all_seq(
