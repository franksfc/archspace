...
from mindspeed_llm.fsdp2.models.common.indexer_loss import IndexerLossLoggingHelper
...
class TrainMonitor:
    ...
    def step(
        ...
        Args:
            ...
            sparse_attn_reduce_group: Process group used to reduce sparse-attention metrics
        ...
        flops_mfu_metrics = self._compute_flops_mfu_metrics(batch_seqlens, elapsed_time)

        # 3.5 Throughput Metrics (token/s/sequence length stats)
        throughput_metrics = self._compute_throughput_metrics(batch_seqlens, elapsed_time, batch_size)

        # 3.6 Sparse Attention Metrics
        sparse_attn_metrics = self._compute_sparse_attn_metrics(
            loss_scale=1.0,
            iteration=current_step,
            reduce_group=sparse_attn_reduce_group,
        )
    ...
    # ------------------------------ Metrics method ------------------------------
    @staticmethod
    def _compute_training_progress_metrics(
        epoch: int, lr_scheduler, current_step: int, batch_size: int
    ) -> Dict[str, Any]:
        """
    ...
    def _compute_flops_mfu_metrics(self, batch_seqlens: List[int], elapsed_time: float) -> Dict[str, float]:
        ...
        return {"flops_achieved(T)": flops_achieved, "flops_promised(T)": flops_promised, "mfu": mfu * 100}

    def _compute_throughput_metrics(
        self, batch_seqlens: List[int], elapsed_time: float, batch_size: int
    ) -> Dict[str, float]:
        """
        ...
        batch_tokens, real_batch_size = all_reduce((batch_tokens, real_batch_size), op="sum")
    ...
    @staticmethod
    def _compute_sparse_attn_metrics(loss_scale, iteration, writer=None, wandb_writer=None, reduce_group=None):
        return IndexerLossLoggingHelper.track_indexer_metrics(
            loss_scale,
            iteration,
            writer=writer,
    ...
    def _generate_and_log_metrics(self, metrics: Dict[str, Any], total_steps: int, elapsed_time_per_iteration: float):
        ...
        if self.training_args.log_throughput:
            log_string += self.log_templates["throughput"].format(metrics["tokens_per_second"], metrics["mfu"])
        ...
        if "indexer_loss" in metrics:
            log_string += self.log_templates["sparse_attn"].format(metrics["indexer_loss"])
        ...
        log_string += self.log_templates["npu_memory"].format(
            metrics["max_memory_allocated(GB)"], metrics["max_memory_reserved(GB)"]
        )
        ...
        if hasattr(self.training_args, 'log_cpu_memory') and self.training_args.log_cpu_memory:
            log_string += self.log_templates["cpu_memory"].format(
                metrics["cpu_used_memory(GB)"], metrics["cpu_available_memory(GB)"], metrics["cpu_memory_usage(%)"]
            )
    ...
    def _get_empty_logging_state() -> Dict[str, Any]:
        ...
        """
        return {"logged_step": 0, "logged_loss": 0.0, "time": time.time()}
