...
class TrainingBasicFeature(MindSpeedFeature):
    ...
    def pre_validate_args(self, args):
        ...
        if (
            (args.attention_mask_type == 'general' and not reset_data)
            or alibi_without_flash_attn
    ...
    def post_validate_args(self, args):
        ...
        if args.num_layers_per_virtual_pipeline_stage and args.num_layer_list:
            raise AssertionError('num_layers_per_virtual_pipeline_stage is not support work with num_layer_list')
        ...
        print_rank0_by_args(
            args,
            f'vpp_size would be {args.virtual_pipeline_model_parallel_size} since '
            f'num_layers_per_virtual_pipeline_stage is {args.num_layers_per_virtual_pipeline_stage}',
        )

    def register_args(self, parser: ArgumentParser):
        group = parser.add_argument_group(title=self.feature_name)
        group.add_argument('--jit-compile', action='store_true', default=False, help='Setting jit compile mode to True')
        group.add_argument(
