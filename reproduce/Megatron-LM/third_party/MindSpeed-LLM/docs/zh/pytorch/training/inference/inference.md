...
2. 获取启动脚本
    ...
    使用LLaMA2模型目录下的[流式推理脚本](../../../../../examples/mcore/llama2/generate_llama2_13b_ptd.sh)。
...
3. 编辑示例脚本
    ...
    > [!NOTE]
    >
    > 除了路径的配置，推理脚本中的其他参数应与训练时的参数保持一致（例如，TP/PP/EP/VPP等并行切分，`--noop-layers`和`--num-layer-list`等自定义模型结构参数），否则会出现模型权重加载失败的情况。
