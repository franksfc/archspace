...
def olmo3_hsdp_inter_overlap_backward_enabled() -> bool:
    """Return whether the second HSDP reduction overlaps backward.

    This is the production TP2 behavior. The flag changes only collective
    scheduling: gradients still follow the exact intra-instance
    reduce-scatter followed by inter-instance all-reduce arithmetic.
    """

    raw_value = os.getenv("OLMO3_HSDP_INTER_OVERLAP_BACKWARD", "0").strip()
    if raw_value not in ("0", "1"):
        raise RuntimeError(
            "OLMO3_HSDP_INTER_OVERLAP_BACKWARD must be exactly 0 or 1, "
            f"got {raw_value!r}"
        )
    return raw_value == "1"


def olmo3_hsdp_inter_tp_lane_pack() -> int:
    """Return the supported TP-lane packing factor."""

    raw_value = os.getenv("OLMO3_HSDP_INTER_TP_LANE_PACK", "1").strip()
    try:
        pack = int(raw_value)
    except ValueError as exc:
        raise RuntimeError(
            "OLMO3_HSDP_INTER_TP_LANE_PACK must be exactly 1 or 2, "
            f"got {raw_value!r}"
        ) from exc
    if pack not in (1, 2):
        raise RuntimeError(
...
class _Olmo3WorkList:
    """Small ``Work``-compatible wrapper used by TP-lane pack2."""

    def __init__(self, works):
        if not works:
            raise RuntimeError("an OLMo3 HSDP work list cannot be empty")
...
def olmo3_hsdp_tp_lane_pack_reduce(
    bucket_group,
    local_data_view: torch.Tensor,
    reduce_op: torch.distributed.ReduceOp,
) -> _Olmo3WorkList:
    """Reduce two node-local TP lanes through the TP-rank-zero inter group."""

    tp_group = bucket_group.olmo3_tp_lane_pack_group
    if tp_group is None or bucket_group.olmo3_tp_lane_pack_tag is None:
        raise RuntimeError("OLMo3 HSDP TP-lane pack2 state is missing")
    tp_members = torch.distributed.get_process_group_ranks(tp_group)
    if len(tp_members) != 2:
        raise RuntimeError("OLMo3 HSDP TP-lane pack2 requires TP=2")
    tp_rank = torch.distributed.get_rank(group=tp_group)
    if tp_members[tp_rank] != torch.distributed.get_rank():
        raise RuntimeError("OLMo3 HSDP TP-lane pack2 rank order is inconsistent")

    is_leader = tp_rank == 0
    peer_global_rank = tp_members[1 if is_leader else 0]
    gather_tag = bucket_group.olmo3_tp_lane_pack_tag
    scatter_tag = gather_tag + 1
    shard_elements = local_data_view.numel()

    if is_leader:
        if bucket_group.olmo3_tp_lane_pack_buffer is None:
            raise RuntimeError("OLMo3 HSDP TP-lane pack2 staging buffer is missing")
        packed = bucket_group.olmo3_tp_lane_pack_buffer[: 2 * shard_elements].view(
            2, shard_elements
        )
        packed[0].copy_(local_data_view)
        torch.distributed.irecv(
            packed[1], src=peer_global_rank, tag=gather_tag
        ).wait()
        torch.distributed.all_reduce(
            packed,
            op=reduce_op,
            group=bucket_group.inter_distributed_optimizer_instance_group,
            async_op=True,
        ).wait()
        local_data_view.copy_(packed[0])
        work = _Olmo3WorkList(
            [
                torch.distributed.isend(
                    packed[1], dst=peer_global_rank, tag=scatter_tag
                )
            ]
        )
    else:
        torch.distributed.isend(
            local_data_view, dst=peer_global_rank, tag=gather_tag
        ).wait()
        work = _Olmo3WorkList(
            [
                torch.distributed.irecv(
                    local_data_view, src=peer_global_rank, tag=scatter_tag
                )
            ]
        )

    # One staging buffer is shared across bucket groups. Complete this P2P
    # transfer before a later bucket reuses it.
    work.wait()
    return work
...
class _ParamAndGradBucketGroup:
    ...
    def __init__(
        ...
        if self.ddp_config.num_distributed_optimizer_instances > 1:
            ...
            self.olmo3_tp_lane_pack_group = None
            self.olmo3_tp_lane_pack_buffer = None
            self.olmo3_tp_lane_pack_tag = None
    ...
    def start_param_sync(self, force_sync: bool = False):
        ...
        if self.intra_distributed_optimizer_instance_size == 1:
            # The optimizer owns the full parameter buffer in this instance;
            # a size-one all-gather is a pure launch dependency.
    ...
    def start_grad_sync(self):
        ...
        # Partial DistOpt has a strict RS -> inter-instance AR dependency.
        # Keep the two handles separate so the RS can be issued by the
        # autograd hook and the AR can overlap the remaining backward work.
        ...
        if replicated_distopt_instance:
            # RS(size=1) is a no-op. Reduce the complete gradient directly
            # over replicas while the bucket is ready.
        ...
        # Synchronous partial DistOpt retains the stock RS -> AR sequence.
        # The overlap path defers AR dispatch to ``start_inter_grad_sync``.
        if partial_distopt and not async_op:
            assert self.inter_distributed_optimizer_instance_group is not None
            with stream_context, _coalescing_manager(
                self.inter_distributed_optimizer_instance_group, async_ops=False
            ) as cm:
                for bucket in self.buckets:
                    local_data_view = shard_buffer(
                        bucket.grad_data, self.intra_distributed_optimizer_instance_size
        ...
        if async_op and partial_distopt:
            self.intra_grad_reduce_handle = cm
        elif async_op:
    ...
    def start_inter_grad_sync(self):
        """Launch the inter-instance HSDP reduction after its local RS."""
        ...
        assert self.intra_grad_reduce_handle is not None, (
            f'Intra-instance communication has not been issued for this bucket '
            f'({len(self.params_with_grad)}/{len(self.params)} params have grad available)'
        )
        assert self.grad_reduce_handle is None, (
            'Inter-instance communication has already been issued for this bucket'
        )

        stream_context = torch.cuda.stream(self.communication_stream)
        with stream_context:
            # HCCL process groups may own distinct internal streams. Work.wait
            # establishes the exact RS -> AR dependency without synchronizing
        ...
        tp_lane_pack = olmo3_hsdp_inter_tp_lane_pack()
        if tp_lane_pack == 1:
            with stream_context, _coalescing_manager(
            ...
            with stream_context:
                cm = olmo3_hsdp_tp_lane_pack_reduce(
                    self,
                    local_data_view,
                    reduce_op,
                )
