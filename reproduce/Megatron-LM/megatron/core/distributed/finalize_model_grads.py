...
def _allreduce_layernorm_grads(model: List[torch.nn.Module], config: TransformerConfig):
    ...
    if parallel_state.get_tensor_model_parallel_world_size() > 1 and (
        ...
        for model_chunk in model:
            ...
            for name, param in get_attr_wrapped_model(model_chunk, 'named_parameters')():
                is_replicated_qk_norm = (
                    ('q_layernorm' in name or 'k_layernorm' in name)
                    and not getattr(param, 'tensor_model_parallel', False)
                )
                if param.requires_grad and (
                    getattr(param, 'sequence_parallel', False)
                    or is_replicated_qk_norm
                ):
                    params.append(param)
...
def _allreduce_num_tokens(
    num_tokens: torch.Tensor,
    config: TransformerConfig,
) -> None:
    """Produce the exact global token count for packed CP training."""

    finalize_deferred = getattr(
        config,
        "olmo3_finalize_deferred_num_tokens_func",
        None,
    )
    if callable(finalize_deferred) and finalize_deferred(num_tokens):
        return

    if (
        getattr(config, "olmo3_cp_aggregated_num_tokens", False)
...
def _broadcast_num_tokens_across_pipeline(
    num_tokens: torch.Tensor,
) -> list[torch.Tensor]:
    """Broadcast token counts for real PP groups, skipping singleton groups."""
    ...
    for lr, group in zip(last_rank, pp_group):
        # A PP=1 group contains exactly one rank, so this broadcast is a
        # mathematical no-op. Avoid constructing a separate lazy HCCL
        # communicator for every singleton PP group: its root listener can
        # collide with an outbound socket even though no data needs to move.
...
def finalize_model_grads(model: List[torch.nn.Module], num_tokens: Optional[torch.Tensor] = None):
    ...
    if num_tokens is not None:
        ...
        num_tokens_list = _broadcast_num_tokens_across_pipeline(num_tokens)
        assert all(x.item() == num_tokens_list[0] for x in num_tokens_list)

        _allreduce_num_tokens(num_tokens, config)
        for model_chunk in model:
            if num_tokens > 0:
