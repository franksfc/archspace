...
def _get_write_results_queue():
    ...
    if _results_queue is None:
        if os.environ.get("MCORE_DIST_CKPT_NO_FORK", "0") == "1":
            # The no-fork save path executes preloading, writing, and
            # finalization in the training process.  A process-local queue is
            # therefore sufficient and, unlike multiprocessing.Manager,
            # cannot spawn a server process after HCCL/NPU initialization.
            ...
            if (
                ...
                logger.info(
                    "MCORE_DIST_CKPT_NO_FORK is enabled; using an in-process "
                    "checkpoint results queue"
                )
        else:
...
class FileSystemWriterAsync(FileSystemWriter):
    ...
    def write_preloaded_data_multiproc(
        ...
        # Forking writer grandchildren after HCCL/NPU initialization can deadlock
        # even when the checkpoint tensors have already been staged on CPU.  Keep
        # the outer async writer process, but allow its file buckets to be written
        # sequentially in that process.  This is also the safe path for synchronous
        # torch_dist checkpoints, whose implementation waits for the outer writer.
        if os.environ.get("MCORE_DIST_CKPT_NO_FORK", "0") == "1":
            for local_proc_idx, write_bucket in enumerate(write_buckets):
                try:
                    write_results_or_exc[local_proc_idx] = (
                        FileSystemWriterAsync._write_preloaded_bucket(
                            transform_list, write_bucket, use_fsync=True
                        )
                    )
                except Exception as exc:
                    logger.exception(
                        "FileSystemWriterAsync direct writer failed for bucket %s",
                        local_proc_idx,
                    )
    ...
    @staticmethod
    def _write_preloaded_bucket(
        transform_list, write_bucket: WriteBucket, use_fsync: bool
    ) -> List[WriteResult]:
        """Write one CPU-staged bucket and return its storage results."""
    ...
    def write_preloaded_data(
        ...
        try:
            local_results = FileSystemWriterAsync._write_preloaded_bucket(
                transform_list, write_bucket, use_fsync
            )
