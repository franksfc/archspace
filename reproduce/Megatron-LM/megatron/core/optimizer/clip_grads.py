...
GradStatsParallelGroup = Union[
    Optional[torch.distributed.ProcessGroup],
    List[Optional[torch.distributed.ProcessGroup]],
    Tuple[Optional[torch.distributed.ProcessGroup], ...],
]
...
def _all_reduce_grad_stats(
    ...
    """Reduce a statistic over one group or orthogonal process groups."""

    if isinstance(grad_stats_parallel_group, (list, tuple)):
        if not grad_stats_parallel_group:
            raise ValueError("grad_stats_parallel_group cannot be empty")
        groups = grad_stats_parallel_group
    else:
        groups = (grad_stats_parallel_group,)
    for group in groups:
